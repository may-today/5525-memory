...
import { PortalBookingButton } from "@/components/portal-booking-button";
...
export function SharePage() {
	...
	const selectedShowIndexes = useSelector(concertStore, (state) =>
		state.selectedShows.map((show) => show.showIndex),
	);
	...
	const signatureSong = useMemo(() => {
		...
		if (data.songStats.topSong)
			return { isRandomPick: false, title: data.songStats.topSong.title };
		return null;
	}, [data]);
	...
	async function copyPasscode() {
		...
		toast({
			title: "口令已复制",
			description: "把它粘贴到其他小程序，即可保存这份回忆。",
		});
	}
	...
	return (
		<div className="mx-auto flex min-h-svh w-full max-w-md flex-col p-6">
			<div className="mb-5">
				...
				<h2 className="mt-1.5 font-semibold font-title text-xl">
					时空旅行纪念品
				</h2>
			</div>
			...
			<div className="mt-6 flex flex-col gap-3">
				...
				<Sheet>
					<SheetTrigger
						disabled={!passcode}
						render={<Button className="w-full" size="lg" variant="glass" />}
					>
						将场次保存到...
					</SheetTrigger>
					<SheetContent className="max-h-[80svh] rounded-t-2xl" side="bottom">
						...
						<div className="flex flex-col gap-3 px-5 py-5">
							...
							<p className="text-muted-foreground text-xs">
								口令只记录你选中的场次，不包含昵称、位置或其他个人资料。
							</p>
							<section
								aria-labelledby="supported-apps-title"
								className="mt-2 w-full border bg-muted/40 p-4"
							>
								<p
									className="text-muted-foreground text-xs"
									id="supported-apps-title"
								>
									目前支持的应用
								</p>
							...
							<p className="text-muted-foreground/50 text-xs leading-5">
								...
								<a
									className="underline"
									href="https://github.com/wmbkapp/5525-memory"
									rel="noopener"
									target="_blank"
								>
									开源
