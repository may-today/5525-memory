...
function buildCityGroups(allShows: Show[]): CityGroup[] {
	...
	groups.sort(
		(a, b) =>
			...
			a.shows[0].showDate.localeCompare(b.shows[0].showDate),
	);
	return groups;
}
...
function getFormShowThemeColor(show: Show): string {
	return (
		FORM_SHOW_THEME_COLORS[
			show.subTheme as keyof typeof FORM_SHOW_THEME_COLORS
		] ?? show.themeColor
	);
}
	...
	return (
		<div className="px-5 pt-8 pb-6">
			<div className="mb-2 flex items-baseline justify-between gap-4">
				...
				<p className="font-geist text-muted-foreground text-xs tabular-nums">
					{padStopNumber(step)} / 02
				</p>
			</div>
...
export function FormPage() {
	...
	const selectedIds = useMemo(
		() => new Set(selectedShows.map((show) => show.id)),
		[selectedShows],
	);
	const selectedCityCount = useMemo(
		() => new Set(selectedShows.map((show) => show.city)).size,
		[selectedShows],
	);
	...
	function handleLocationCityChange(value: string) {
		if (value === LOCATION_NONE || value === LOCATION_OTHER) {
			updateConcertProfile({
				city: value === LOCATION_OTHER ? LOCATION_OTHER : "",
				coordinates: null,
			});
			return;
		}
	...
	function handleImportPasscode() {
		...
		const showsByIndex = new Map(
			allShows.map((show) => [show.showIndex, show]),
		);
	...
	if (step === "profile") {
		return (
			<div
				className="form-page form-step-in flex min-h-svh flex-col"
				key="profile"
			>
				<FormStepHeader
					eyebrow="Passenger"
				...
				<div className="flex flex-1 flex-col gap-8 px-5 pb-4">
					<div className="space-y-3">
						...
						>
							...
							<span className="text-[9px] text-muted-foreground/60 uppercase tracking-[0.25em]">
								可选
							</span>
						</label>
						<Input
							...
							maxLength={24}
							onChange={(event) =>
								updateConcertProfile({ nickname: event.target.value })
							}
							placeholder="请输入昵称"
							type="text"
					...
					<div className="space-y-3">
						...
						<Select
							...
							onValueChange={(value) =>
								handleLocationCityChange(value ?? LOCATION_NONE)
							}
		...
		<div className="form-page form-step-in flex min-h-svh flex-col" key="shows">
			<FormStepHeader
				action={
					<div className="flex shrink-0 items-center gap-3 pb-1">
						<Sheet
							onOpenChange={setIsPasscodeSheetOpen}
							open={isPasscodeSheetOpen}
						>
							<SheetTrigger
								render={<Button size="xs" type="button" variant="glass" />}
							>
								导入口令
							</SheetTrigger>
							<SheetContent
							...
							>
								<SheetHeader className="border-b px-5 pt-6 pb-4">
									...
									<SheetDescription>
										粘贴其他应用保存的 5525 场次口令，即可恢复已选场次。
									</SheetDescription>
								</SheetHeader>
								<div className="px-5 py-5">
									<label
										className="flex flex-col gap-2"
										htmlFor="show-passcode"
									>
			...
			<div className="flex flex-1 flex-col gap-2 px-4 pb-4">
				{CITY_GROUPS.map((group) => {
					...
					const selectedCount = group.shows.filter((s) =>
							...
							>
								<div className="-ml-2 flex flex-1 items-center gap-1.5">
									<img
										...
										className="size-10"
										height="40"
										src={getCityIcon(group.city)}
										width="40"
									/>
									<span className="font-bold font-title text-base tracking-wide">
										{group.city}
									</span>
									<span className="text-muted-foreground text-xs">
										· 共 {group.shows.length} 场
									</span>
								</div>
								{selectedCount > 0 && (
									...
									{group.shows.map((show) => {
										...
										return (
											<button
												...
												style={
													{
														"--show-color": getFormShowThemeColor(show),
													} as CSSProperties
												}
												type="button"
											>
												...
												<span className="form-show-check flex size-[17px] flex-shrink-0 items-center justify-center">
													<Check
														className="form-show-check-icon size-3 text-zinc-950"
														strokeWidth={3.5}
													/>
												</span>
				...
				<div className="mb-2.5 flex items-center justify-between gap-3">
					{totalCount === 0 ? (
						...
						<p className="flex items-baseline gap-1 text-[11px] text-muted-foreground">
							已选
							<span className="form-selected-count text-base tabular-nums leading-none">
								{totalCount}
							</span>
							场 · {selectedCityCount} 座城市
						</p>
					)}
					{totalCount > 0 && (
						<button
