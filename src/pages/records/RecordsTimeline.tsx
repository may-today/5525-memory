 ...
 */
export function RecordsTimeline({ allShows, onDetailOpenChange }: RecordsTimelineProps) {
  ...
  return (
    <>
      <div className="mt-8">
        <p className="text-[10px] text-zinc-500 uppercase tracking-[0.3em]">Tour Archive</p>
              ...
              {group.stops.map((stop, stopIndex) => {
                ...
                return (
                  <li className="records-stop relative pl-9" key={stop.shows[0].id} style={railStyle}>
                    <header>
                      <p className="records-stop-no font-title">NO.{String(stop.index + 1).padStart(2, '0')} 站</p>
                      ...
                      {stop.shows.map((show) => (
                        <li className="flex items-center gap-3 py-2.5" key={show.id}>
                          <div className="flex min-w-0 flex-1 flex-wrap items-baseline gap-x-2.5 gap-y-0.5">
                            ...
                            <span className="text-[10px] text-zinc-600">{getDayAbbr(show.showDate)}</span>
      ...
      <p className="mt-16 text-center text-xs text-zinc-600">不说再见 未来再见</p>
