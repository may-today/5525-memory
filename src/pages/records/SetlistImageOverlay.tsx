 ...
 * 图床是微博（sinaimg.cn）带 Referer 防盗链，图片必须以 no-referrer 加载；
 * 失败时提供 rel="noreferrer" 的原图链接兜底。
 */
export function SetlistImageOverlay({ onClose, show }: SetlistImageOverlayProps) {
    ...
    >
      <header className="flex shrink-0 items-center justify-between gap-3 border-white/10 border-b px-5 pt-[max(1.25rem,env(safe-area-inset-top))] pb-4">
        <div className="min-w-0">
          <p className="text-[10px] text-zinc-500 uppercase tracking-[0.3em]">Setlist</p>
          <h2 className="mt-0.5 truncate font-title text-base text-zinc-100">
            {show.city} {show.dayLabel} · <span className="font-geist">{show.dateSlash}</span>
          </h2>
        </div>
        <button
          aria-label="关闭歌单"
      ...
      <div className="records-overlay-body min-h-0 flex-1 overflow-y-auto overscroll-contain">
        {loadState === 'loading' && (
          <p className="animate-pulse py-24 text-center text-sm text-zinc-500">正在展开歌单长图…</p>
        )}
        {loadState === 'error' && (
        ...
        <img
          ...
          className={loadState === 'error' ? 'hidden' : 'mx-auto h-auto w-full max-w-md'}
          height={5}
          onError={() => setLoadState('error')}
          onLoad={() => setLoadState('ready')}
          referrerPolicy="no-referrer"
          src={show.playlistImg}
          width={3}
        />
