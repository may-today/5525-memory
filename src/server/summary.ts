...
function hasValidCoordinates(
	coordinates: LocationCoordinates | null,
): coordinates is LocationCoordinates {
	return (
		coordinates !== null &&
 ...
 */
function isRandomSong(
	item: SummarySetlistItem,
...
const SONG_DECORATION_PATTERN =
	/\p{Extended_Pictographic}|\p{Regional_Indicator}|\p{Emoji_Modifier}|\u200D|\uFE0E|\uFE0F|\u20E3/gu;
...
function expandSongItems(items: SummarySetlistItem[]): SummarySetlistItem[] {
	...
	for (const item of items) {
		...
		const titles =
			item.itemType === "medley"
				? item.title.split(MEDLEY_SONG_SEPARATOR_PATTERN)
				: [item.title];
...
function buildSongStats(items: SummarySetlistItem[]): SongStats {
	...
	const entries = [...countTitles(songs)].map(([title, count]) => ({
		title,
		count,
	}));
...
function getSongTitleKey(title: string): string {
	return stripSongDecorations(title)
		.normalize("NFKC")
		.replace(SONG_COMPARISON_NOISE_PATTERN, "")
		.toLocaleLowerCase();
}
...
const MAYDAY_SONG_TITLE_KEYS = new Set(songList.map((song) => getSongTitleKey(song.title)));
const MAYDAY_SONG_BASE_TITLE_KEYS = new Set(songList.map((song) => getSongBaseTitleKey(song.title)));

/** Returns whether a setlist title matches a 五月天 song in the maintained catalog. */
function isMaydayCatalogSong(title: string): boolean {
	const titleKey = getCanonicalSongTitleKey(title);
	return MAYDAY_SONG_TITLE_KEYS.has(titleKey) || MAYDAY_SONG_BASE_TITLE_KEYS.has(titleKey);
}

/** Normalizes a raw setlist_items.section value into the coarse type an appearance entry reports. */
function getSongSectionType(
	section: string,
): TourSongAppearance["sectionType"] {
...
function buildTourSongs(
	...
	for (const song of songList) {
		...
		if (!catalogByBaseTitleKey.has(baseTitleKey))
			catalogByBaseTitleKey.set(baseTitleKey, song);
	}

	for (const item of expandSongItems(items)) {
		const titleKey = getCanonicalSongTitleKey(item.title);
		const song =
			catalogByTitleKey.get(titleKey) ?? catalogByBaseTitleKey.get(titleKey);
...
function buildRandomSongStats(
	items: SummarySetlistItem[],
	showsById: Map<number, Show>,
): RandomSongStats {
	const randomSongs = expandSongItems(items).filter((item) =>
...
function buildSeasonalSongStats(
	...
	const countsBySeason = SEASON_MONTH_GROUPS.map(
	...
	const seasons = countsBySeason.map((counts, seasonIndex) => {
		...
		return {
			showCount: selectedShows.filter(
				(show) => getSeasonIndex(show.showDate) === seasonIndex,
			).length,
			song: top ?? null,
		};
	});
...
function buildRareSongStats(
	...
	for (const item of randomSongs) {
		...
		if (existing) {
			...
			heardByTitle.set(item.title, {
				heardCount: 1,
				heardCity: show.city,
				heardDate: show.showDate,
			});
		}
	}
	...
	const ranked = [...heardByTitle.entries()]
		...
		.sort(
			(a, b) =>
				a.heardCount - b.heardCount ||
				a.tourCount - b.tourCount ||
	...
	const entries = ranked
		.filter(
			(entry, index) =>
				index === 0 || entry.tourCount <= RARE_SONG_TOUR_COUNT_MAX,
		)
...
export const getSummaryData = createServerFn({ method: "POST" })
	...
	.handler(
		async ({ data: { showIds, city, coordinates } }): Promise<SummaryData> => {
			...
			const selectedSetlistItems = setlistItems.filter((item) =>
				selectedShowIdSet.has(item.showId),
			);
			...
			return {
				...
				rareSongStats: buildRareSongStats(
					setlistItems,
					selectedShowIdSet,
					showsById,
				),
				seasonalSongStats: buildSeasonalSongStats(
					selectedSetlistItems,
					selectedShows,
					showsById,
				),
