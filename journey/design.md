...
项目已迁移至 TanStack Start，完整页面流程可以正常跳转。表单使用真实场次目录，并为后续统计流程保存用户选择的场次 ID。统计页面使用 D1 真实数据生成场次、城市、歌曲、随机曲目（专属歌单 + 最小众歌单）和嘉宾统计；报告页已从 mock 效果图升级为 TanStack AI 驱动的结构化报告卡片，基于用户已选场次调用受控统计工具生成结果；分享页已从占位预览区升级为「星轨共振」宇宙航线海报，并可导出 PNG 图片。
...
场次选择底栏提供「全选」与「清空」批量操作（2026-07-16）；「全选」以当前可见场次目录原子替换选择并在全部选中时禁用，「清空」仅在已有选择时显示，两者都会使旧选择对应的匿名报告登记失效。

`/share` 另提供「将场次保存到...」底部 Sheet：它只把选中场次转为可复制的短口令，不携带昵称、位置或其他资料。口令直接以完整场次目录按 `show_date ASC, id ASC` 排列后的零基 `showIndex` 为坐标（非正式活动场次虽不可见，仍占用编号），使用 `5525-` 前缀加自适应压缩：稀疏选择为 LEB128 差分、密集选择为位图，二者都做无填充 Base64URL 并取较短结果；`showIndex` 会随已选场次写入 localStorage，分享页不必加载全量场次，第三方也可直接得到序号后自行查询资料。由于 SSR 无法读取浏览器 localStorage，分享页会在客户端挂载后恢复持久化的 `showIndexes`，令刷新后的入口保持可用（2026-07-12）。

`/share` 操作区顶部另有「预约 任意门启航版」预告入口（2026-07-14，开放时间接口化于 2026-07-16）：开放时间通过 TanStack Start server function 从 `https://mayday.bandchina.com/api/memoir-config/public` 的 `memoir_open_at` 获取（目标接口未开放浏览器 CORS），无时区值按北京时间解释；客户端据此每秒更新倒计时，归零后显示「预约现已开启」，底部日期文案也从同一值格式化。接口失败或字段非法时显示开放时间待定，不回退到可能过期的硬编码时间。尚未提供外部预约地址，当前点击只展示介绍；接入正式链接后替换为跳转行为。
...
`src/data/song-filter.ts` 的 `randomSongBlackList` 中的主题固定曲；随后仅对
`section = 'request'` 排除同文件 `randomSongRequestBlackList` 中的点歌固定曲；最后
按 `randomSongSpecialBlackList` 排除特定主题与城市组合的固定曲（目前为 `5525` 台中场的
〈如果我们不曾相遇〉）。
其中「最小众歌单」（`SummaryCardRareSongs`）会再限制为能匹配
`src/data/song-list.ts` 五月天曲库的歌曲；嘉宾／翻唱等非五月天曲目不参与该卡的
专属曲目与冷门排序。报告页的 request/encore 歌曲排行与时间线采用同一排除规则；主歌单和结尾曲等非随机口径不受影响。
...
- **主要内容**：sticky hero（多巴胺弥散光背景 + Canvas 2D 全屏粒子隧道 + 聚字后淡入的文案「穿过漫长星轨，在 5525 的时空里 / 你与五月天陪伴了 XXXX 分钟」，数字 Doto + 蓝辉光、**无千位逗号**，副行「唱过 XX 首歌吧」（统计用户选中场次的全部歌曲，不限种类），文案块位于 `top-[58%]`）；顶部引导语「耳机里的少年，已经唱了 25 年。」「这一趟疯狂世界，你又是何时跳上了这班列车？」「拉开时光机的舱门，轻轻往下拨动，开启你的 5525 穿梭航线。」（滚动启程后经滞回阈值淡出，滚回起点再淡入，`.summary-duration-intro` + 引擎 `onIntroChange`）；hero 下方为逐场时长列表（黑胶卡点线引导排版），合计行与收尾句「时光机随时待命，这条航线永远可以再飞一遍」。
...
- **组件**：`SummaryCardCity`
- **设计目标（2026-07-16 由 cobe 地球改版，见 `journey/plans/2026-07-16-city-passport-redesign.md` 与 `journey/plans/2026-07-16-city-passport-polish.md`）**：巡演城市高密度集中在中国大陆，3D 地球投影点位重叠不可读；改为「巡演护照」横滑卡组——一城一枚纯 SVG 护照印章，去过的城市盖章点亮，未去过的呈灰色虚线「还没盖上的护照页」。签名元素是**盖章瞬间**：城市页首次滑入视口时印章以 overshoot 缓动「盖下去」（scale 1.45→1 + 回转微倾角 + 透明度 0→1），邮戳日期条延迟 0.45s 落下。
- **护照首页（卡组 index 0）**：Doto 大字分数「已盖章数 / 总城市数」+ 同行基线对齐的说明（已盖章数白色 + 蓝辉光）→ 6 列城市纪念章网格 → 底部两行 MRZ 机读码（内容为真实数据：巡演名 / 盖章数 / 总城市数 / 里程，非装饰乱码）。原先的 mono eyebrow `MAYDAY #5525 · TOUR PASSPORT` 已上提为内页框的页签（见「护照内页边框」）。
- **城市纪念章网格（2026-08-05，见 `journey/plans/2026-08-05-passport-index-landmarks.md`）**：首页原本 5 列圆章内只排城市名，整页纯文字。改为复用 `/form` 场次选择页的同一套城市地标 3D 小图（`src/lib/city-icon.ts` → CDN 80×80 webp，23 个城市名与映射表一一对应；新增 `hasCityIcon` 供无图城市退回纯文字圆章），一枚章 = 地标图坐在子主题墨色圆框里、章下城市名。**签名元素是「未上墨的凹版」**：未到场城市用同一张地标图 `grayscale + brightness(0.85) + opacity 0.32`，轮廓仍在、颜色未落，比「有图/没图」更贴盖章隐喻。只有章体带微倾角与错峰 pop-in，城名保持水平；去过 2 场以上的城市在城名后跟 Doto 场次数（与大印章「及另外 N 场」同口径，23 格里清一色的「1」是噪声）；5 字以上长名（拉斯维加斯）降一档字号保证单行。列数由 5 改 6：城名移到章外后每行多约 14px，6 列 4 行才能在 667px 高的小屏上与分数、MRZ 一起放进内页框。
- **印章构成**（纯 CSS/SVG，不依赖图片资产；`src/data/stadium.ts` 的场馆图映射保留未引用）：外圈细环 r=92 + 机刻细密刻线圈 r=87（dasharray 细点）+ 环形文字 r=80（`subTheme · versionName · venue` 沿 textPath 重复，50s/转，`data-paused` 暂停）+ 内环 r=68 + 章体 r=58；中心城市名（`font-title`）；到场章整体叠 radial-gradient mask 模拟印泥不匀（复用小众歌单卡「仅此一次」印章手法）；每枚印章按序号取 ±4° 内固定微倾角（`--stamp-rotation`）；邮戳日期条压在 66% 高度斜穿印章下部（不盖住城市名），Doto 日期 + 「及另外 N 场」。
- **彩虹印台渐变墨色（2026-07-16）**：23 个城市里 10 个跨多个子主题（北京、台北横跨全部三段），单一代表色会抹掉这层信息。`CityMarker.subThemes` 返回该城市全部场次的去重子主题（过滤空值脏数据，如香港某场），客户端按 粉→蓝→橙 固定色序映射为多段 SVG `linearGradient`（`--stamp-ink` 传 `url(#…)`，单主题退化为纯色），描边/环形文字/章体共用；辉光、邮戳、dots 取首色（drop-shadow 无法渐变）；迷你章底色用同一渐变、描边保持首色（小尺寸渐变描边会糊）。现实中「彩虹印台」真实存在，隐喻成立。
- **环境光**：卡片背景叠 10% 透明度 radial 光晕，颜色随当前页切换（首页蓝 / 已盖章城市主题色 / 未盖章中性灰），`@property` 注册 `--city-ambient` 实现颜色过渡，老浏览器退化为瞬时换色。
- **护照内页边框（2026-08-05）**：每一页（首页与全部城市页）都包在 `.city-passport-page` 内页框里，让卡组看起来像翻护照本而不是一屏居中的文字：外细线 + 沿四边 7px 的 45° 斜纹防伪底纹 + `inset: 7px` 的内细线，页签骑在上框线中点、用卡片底色 `#09090b` 截断框线（首页 `MAYDAY #5525 · TOUR PASSPORT`，城市页 `PAGE + Doto 页码`）。墨色由每页的 `--page-ink` 驱动（首页全站蓝 / 城市页子主题色 / 未到场中性灰），横滑时四边框线随页换色。防伪带用**同一份斜纹重复四次**再以 `background-size`/`background-position` 定位到四条边，不依赖 `mask-composite`，老 Android WebView 同样可用（沿用既定兼容边界）。内页框靠横向 `margin` 在相邻页之间留出书缝，滚动容器改为 `items-stretch` 让页框撑满卡片高度。
- **交互**：横向 snap 滚动一屏一城（原生触摸滚动与外层纵向切卡天然共存，`|deltaY| > |deltaX|` 判定过滤横滑，无需手势豁免）；进入本卡且纵向切页完成后，首页会自动向右轻探约 15% 卡宽再归位，提示护照可以横滑展开，用户主动触摸／滚轮／点击导航即取消，`prefers-reduced-motion` 下不播放；底部 dots 导航（首页为「书页」圆角短矩形，城市点按子主题首色着色、未到场灰色），dots 行 `pb-20` 给容器悬浮「滑动探索」引导让位。**已知坑：Chrome 在 `scroll-snap-type: x mandatory` 容器上会把原生 smooth 滚动（scrollIntoView / scrollTo）中途取消并停在非 snap 位置**，`scrollToIndex` 与入场轻探都改为 RAF 自绘缓动 + 动画期间临时关 snap，reduced-motion 下翻页瞬时跳转，卸载时 cancelAnimationFrame。
- **盖章状态机**：`stampedIndexes: Set<number>` 记录已被激活过的城市页（IntersectionObserver threshold 0.6 驱动 activeIndex），到场章激活前 `opacity: 0`、首次激活加 `data-stamped` 触发一次性 slam 动画后保持盖章态；reduced-motion 下未激活章保持可见、slam/邮戳/迷你章动画全部停用。
- **头部文案**：标题「你的巡演护照」+ 叙事句（城市数 / 公里数用 `.overview-count` Doto 强调，与时间轴卡计数语汇一致）；出发城市去行政区划后缀，「不透露」等退化为「从家出发」；零选择时引导回上一步。
- **数据来源**：`getSummaryData` 的 `cityMarkers`（全部场次城市去重按时间序；含 `subTheme`/`subThemes`/`themeColor`/`versionName`/`venue`/`showDates`/`visitedShowDates`）与 `mileage`；前端按「去过优先、组内时间序」重排。经纬度与 cobe 已不被本卡使用。
- **数据状态**：已接入真实数据，浏览器走查验证护照首页网格与跳转、双主题城市（台中）渐变印章、未到场灰章（高雄/吉隆坡）、RAF 跨 22 页平滑翻页、dots 状态与环境光切换。

### 4. 岁月音乐墙（岁月留声机）

- **组件**：`SummaryCardSongWall`
- **设计目标**：把「歌单」的范围从用户去过的场次扩展到整个曲库——`src/data/song-list.ts` 的五月天曲库（171 首）与全巡演实际唱过但不在曲库内的曲库外歌曲（`tourSongs` 里 `type = 'surprise'`，如翻唱、串烧、活动限定曲）取并集，让用户看到自己听过的歌之外，还有多少首歌是全巡演唱过但自己没赶上、以及多少首歌这次巡演压根没唱。视觉意象为「岁月留声机」：复古木质唱片架上排满黑胶唱片（封套脊线视角），听过的唱片从架上「抽出来」并点亮（2026-07-12 由砖块墙重设计，见 `journey/plans/2026-07-12-songwall-vinyl-shelf.md`）。
- **主要内容**：标题「曲库内外，这是你的 5525 岁月留声机。」；引导语用三个数字连成一句话，落点改为「五月天曲库里实际唱过的 X 首里，你亲耳听过的 Y 首」；引导语下方是「共振频率 VU 表盘」（`summary-vu-*`，2026-07-12 新增）——与唱片架共用木纹外框的功放面板，琥珀背光 SVG 表盘（真空管暖光意象），指针从左端甩出、过冲回摆后定格在听歌进度角度（**五月天曲库中用户听过的首数 / 五月天曲库中本巡演实际唱过的首数** 映射 `-50deg..50deg`，80% 起红区），百分比读数在指针收摆期间淡入，面板下文案「你与五月天的音乐共振频率」，reduced-motion 下指针静态停在进度角度；一行图例（四种唱片状态的迷你脊线色样）；上层多行唱片架 =「五月天曲库」（本巡演唱过的曲目按曲库原始顺序在前，**全巡演未唱过的灰暗唱片沉在架尾**）；下层单行金色唱片架 =「曲库外的意外惊喜」（横向滚动，复用嘉宾卡 `summary-space-scroller` 的隐藏滚动条 + 两侧 mask，仅在存在曲库外曲目时渲染）；收尾句 + 口径注脚。
- **唱片架构成（签名元素）**：`summary-shelf-*`——木质外框（CSS 渐变胡桃木 + 斜向木纹 + 内嵌高光），架板由容器 `repeating-linear-gradient` 按固定行高 120px 绘制（14px 抽出余量 + 96px 唱片 + 10px 架板；2026-07-12 由 23×88px 调成 17px 宽 ×96px 高，细长比例更接近真实黑胶封套脊背），grid `auto-fill` 自适应列数、无需按行切分数据；脊线用 `writing-mode: vertical-rl` 竖排歌名（CJK 直立、Latin 旋转，title span 需 `display: block` 才能让 nowrap/ellipsis 生效），超长省略、完整名进 `title` 属性。
- **唱片状态**：听过 = `--wall-color` 紫 `#a78bfa`（本卡专属强调色）点亮 + 辉光 + `translateY(-10px)` 抽出，挂载 0.4s 后按架上顺序 35ms/张错峰弹出（过冲收敛，`both` 让延迟期间停在架内；跨上下两架连成一道波）；演唱过没赶上 = 暖褐暗色沉在架内（`nth-child` 三种色调微变避免平铺感）；全巡演未唱 = 冷灰更暗 + 降透明度；曲库外 = 金色 `--wall-gold` `#d9a441`（听过亮金抽出、没赶上暗金）。相邻点亮唱片靠 3px 列距 + 脊线左右 inset 明暗缘区分——辉光半径过大时两张相邻点亮唱片会融成一块「宽唱片」，曾被误判为文字折行。`prefers-reduced-motion` 下抽出动画停用，静态保持抽出 + 辉光。
- **唱片抽出详情（2026-07-13，见 `journey/plans/2026-07-13-songwall-record-detail.md`）**：任意脊线可点击（含未唱的灰唱片）。点击后原脊线 `visibility: hidden` 在架上留出空槽，全屏覆盖层（`data-summary-gesture-exempt` 免疫切页手势）里封套经 **WAAPI FLIP**（`element.animate`，translate + 非均匀 scale，0.52s）从脊线位置飞到画面中央展开为正方形封套（`summary-record-*`，配色沿用脊线五种状态的渐变家族，飞行观感即「同一张唱片被放大」）；落定后封套内容淡入、纯 CSS 黑胶盘（沟槽 + 状态色圆标）从封套上沿探出。下方信息面板依次为：数据生成的「相遇故事」一句话（听过 → 首次 isHeard 场次「YYYY.MM.DD 的{城市}，你们第一次在现场相遇」+ 重逢次数；唱过没赶上 → 响起次数；全巡演未唱 → 安静躺在架上）、「唱过的场次」全列表（`max-h` 内滚 + `overscroll-contain`；行 = showDate + 城市 + dayLabel + 点歌/安可徽章，**用户在场的行用本卡状态色轻染高亮 + 「你在场」标记**）、「把唱片放回架上」按钮。关闭（点背景 / Esc / 按钮）时内容淡出、封套反向 FLIP 飞回脊线原位。封套底部出版信息行来自曲库 meta（`《专辑》 · 年份`），曲库外为「曲库之外 · 意外惊喜」。**关键坑**：隐藏文档（后台标签页）里 Chrome 不派发 WAAPI finish 事件（`finished` promise 也不 resolve），`settleAnimation` 用「onfinish + 超过时长的 setTimeout 兜底」保证 isSettled / onClose 必然发生，否则切走标签页时覆盖层会永久卡死。`prefers-reduced-motion` 下跳过 FLIP 与探盘位移，覆盖层直接淡入淡出。
- **数据来源**：`useSummaryDataContext()` 的 `tourSongs`（含 `appearances` 场次出现记录，详情面板直接消费）与静态导入的 `songList`（脊线标题 + 详情封套的专辑/年份 meta）；在组件内按标题字符串做 `Map` 连接（`tourSongs` 里 `mayday` 类型的 `title` 就是曲库标准标题，见 `buildTourSongs`）。服务端仅为此补充了 `SummaryShowInfo.showDate`。
- **数据状态**：已接入真实数据，浏览器走查验证 14 场选中场次下三种状态、金色惊喜架、抽出动画与桌面/移动两种宽度；唱片详情走查验证紫色听过（4/156 高亮 + 安可徽章）、暗褐未听（0/4 + 点歌徽章）、冷灰未唱（无场次列表）、金色曲库外（1/1）四种封套与开合/中途打断路径。

### 5. 专辑解锁进度

- **组件**：`SummaryCardAlbumProgress`
- **设计目标**：把音乐墙里散落在整轮巡演的「听过」状态，收回到每张正式专辑的创作脉络中；用户不只知道听过多少歌，也能看见哪一段五月天已经被自己逐首解锁。
- **主要内容**：标题「九张专辑，解锁成你的五月天。」；开场总览九张专辑的已听/总首数；随后按发行顺序显示九张专辑卡——封面、专辑名、已听首数、百分比进度条，以及逐首的胶囊标签（已听为 sky 蓝点亮，未听为暗灰）。
- **数据口径**：专辑范围、顺序与封面来自 `src/data/cover.ts` 的 `coverIdMap`；每张专辑的分母是 `songList` 中 `meta.album` 同名的所有曲目，而非本巡演唱过的首数；分子复用音乐墙口径，即同标题 `tourSongs` 的任一 appearance 有 `isHeard = true`。曲库外惊喜曲、精选辑、原声带和无专辑曲目不计入九张专辑进度。
- **交互方式**：内容区带 `data-scroll-container`，复用容器滚动优先切页逻辑；无额外点击状态，专注浏览九张唱片的解锁比例。
- **数据状态**：已接入音乐墙既有 `tourSongs` 状态与静态曲库/封面数据，不新增 D1 查询或前端 payload。
...
- **主要内容**：标题「全巡演最少被唱的歌，偏偏被你撞见。」；顶部一束品牌橙静态追光；主纸条（签名元素）——暖白纸片带两道折痕阴影与半透明橙色和纸胶带，落款「点歌纸条 · 城市 YYYY/MM/DD」（用户第一次听到它的场次）+ WJH 手写感歌名，`tourCount === 1` 时右下角盖一枚旋转橙色描边印章「仅此一次」（radial mask 做印泥不匀）；其下文案「全巡演 N 场，《X》只响起过这一次——而你，就在台下」（tourCount > 1 时改为「只响起过 Y 次——其中 Z 次，你就在台下」）；再往下 2 列小纸条网格（交替 ± 微旋转，歌名 + 「城市 · 全巡演 ×N」）；收尾句「没被唱够的歌，才最像秘密」+ 口径注脚。
- **数据口径**：候选曲目与专属歌单同一随机曲目条件（`item_type = 'song'` 且 `section = 'request'` 或 `LIKE 'encore_%'`），但「全巡演出现次数」统计该五月天歌曲在**所有歌曲段落**（固定主歌单、点歌、安可；串烧已拆歌）中的总出现次数，避免某场随机、其他场固定高频演出的歌曲被误判为冷门；按用户听到次数升序 → 全巡演总出现次数升序 → 标题排序，取前 7（1 主 + 6 小）。**小众门槛**：小纸条要求全巡演出现 ≤ 8 次（约 5% 场次），否则单场用户（人人 heardCount = 1）会把《顽固》这类 ×30+ 常驻曲当成冷门曲展示；主纸条不受门槛限制（用户听过的最冷门一首永远值得展示）。
...
- **视觉方向**：`zinc-950` 深色底，复用嘉宾星球的两层星点背景延续宇宙氛围；所有主题色效果由场次 `themeColor` 通过 `--memory-color` 驱动（日期颜色/光晕、辉光、caption 边线、引用符、占位图渐变）；照片占位框带周期性斜向扫光（transform-only ::after），交替微倾斜。真实封面基本为 3:4 竖幅且清晰度有限，照片框按 3:4 显示并收窄到 `min(70%, 270px)`，条目间左右交替对齐（与倾斜方向配套）避免低清图撑满全宽；相框背后垫「氛围辉光」——同图放大 + `blur(36px)` + 提饱和 + 椭圆 radial mask 柔化边缘（`.summary-memory-photo-echo`），用照片自身的颜色填充四周留白（静态层随照片视差整体移动，模糊只栅格化一次）；caption 是回忆主题，随照片对齐并用标题层级呈现（zinc-100 semibold + 主题色边线，右对齐条目边线换到右侧）。
- **数据来源**：`src/data/special-event.ts` 维护回忆与一个或多个 `YYYY.MM.DD` 场次日期的关联；卡片把日期归一化后与 `SummaryDataContext` 的 `allShows`／`selectedShows` 匹配，`getMemoryGroups` 对任一场次列表按日期升序逐场收集未使用的事件（跨日事件归入最早匹配场次，天然去重），组内保持 `specialEventList` 维护顺序。
- **数据状态**：封面与标题已接入真实运营维护数据；talking 仍为占位，待有可靠的原文来源后再接入。

## 分享页「星轨共振」海报（/share）

- **组件**：`src/pages/SharePage.tsx`（页面壳 + 口令 Sheet 等操作区）+ `src/pages/share/SharePoster.tsx`（海报本体）。
- **定位**：「时空旅行报告」的终点站，一张可收藏、可发朋友圈的「旅行纪念品」——极简艺术海报路线，不堆数字，用一条发光星轨包容所有数据（2026-07-13，见 `journey/plans/2026-07-13-share-star-trail-poster.md`）。
- **海报构成**（固定 5:7 画框，深空底 + 复用嘉宾卡 `summary-space-*` 星点分层背景）：顶部一行档案 eyebrow（`MAYDAY #5525` / `星轨共振 · RESONANCE`）；中部星座画布（签名元素）；下部常驻曲伪声波 + 金句「宇宙很大，但谢谢我们在 5525 号星域相撞。」（WJH + 主导色辉光，5525 用 Doto，`clamp` 字号保证窄屏单行）；底部落款 `N 场 · M 城 · 昵称` + 以场次口令为序列号的 `NO.5525-…`（口令由所选场次唯一决定，天然是「独一无二」的证据；复制入口仍在 Sheet）。
- **星座画布（签名元素）**：场次按日期排序串成发光星轨——x 按序号铺开 + show.id 种子 hash 抖动、y 在中带内漂移（纯函数确定性，同一批场次永远同一张星座）；Catmull-Rom 转贝塞尔平滑连线，`pathLength=1` + dashoffset 做 2.4s draw-in；节点用巡演子主题三色 + 辉光，错峰点亮后持续微 twinkle；标签（MM/DD 城市）>8 场时抽样至 8 个（保留首尾、上下交替避让）。**单场退化为「孤勇星球」**：居中大星球 + 三圈错峰扩散的引力波圆环 + 文案「一颗孤勇星球，自转出独特的引力波」。
- **主导色**：取用户去过最多的子主题色，驱动金句辉光与波形淡染——每个人的海报色温不同。
- **伪声波**：56 根竖条，高度由常驻曲歌名字符 hash × 正弦包络生成、中线镜像；常驻曲 = `randomSongStats.entries[0]`，无随机曲目时回退 `songStats.topSong`（文案改「你听过最多的歌」），都没有时隐藏波形区。
- **数据来源**：`useSummaryData()`（与 /summary 同一 server function，顺带回填 store 解决硬刷新）；口令逻辑不变（store showIndexes → localStorage 回退）。
