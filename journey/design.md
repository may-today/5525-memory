...
- **主要内容**：每条运营维护的场次回忆包含大号日期（主题色 + 光晕）、城市/场馆/场次标签/子主题、回忆封面与描述、一段暂存的 talking 占位文字。封面 URL 为 `//mayday-replay-cdn.ddiu.site/5526-events/{noteId}.webp`，描述直接使用回忆 `title`；条目背后有超大描边幽灵序号和主题色辉光。
...
- **数据来源**：`src/data/special-event.ts` 维护回忆与一个或多个 `YYYY.MM.DD` 场次日期的关联；卡片把日期归一化后与 `SummaryDataContext` 的 `allShows`／`selectedShows` 匹配。跨日回忆在专属范围只显示一次，并按关联场次日期排序。
