...
- **主要内容**：标题「曲库内外，这是你的 5525 岁月留声机。」；引导语用三个数字连成一句话，落点「你亲耳听过的 N 张，已经替你从架上抽了出来」；一行图例（四种唱片状态的迷你脊线色样）；上层多行唱片架 =「五月天曲库」（本巡演唱过的曲目按曲库原始顺序在前，**全巡演未唱过的灰暗唱片沉在架尾**）；下层单行金色唱片架 =「曲库外的意外惊喜」（横向滚动，复用嘉宾卡 `summary-space-scroller` 的隐藏滚动条 + 两侧 mask，仅在存在曲库外曲目时渲染）；收尾句 + 口径注脚。
- **唱片架构成（签名元素）**：`summary-shelf-*`——木质外框（CSS 渐变胡桃木 + 斜向木纹 + 内嵌高光），架板由容器 `repeating-linear-gradient` 按固定行高 112px 绘制（14px 抽出余量 + 88px 唱片 + 10px 架板），grid `auto-fill` 自适应列数、无需按行切分数据；脊线用 `writing-mode: vertical-rl` 竖排歌名（CJK 直立、Latin 旋转，title span 需 `display: block` 才能让 nowrap/ellipsis 生效），超长省略、完整名进 `title` 属性。
