...
`/share` 操作区顶部另有「预约 任意门启航版」预告入口（2026-07-14）：以北京时间 `2026-07-17T19:00:00+08:00` 为目标，在客户端每秒更新倒计时，归零后显示「预约现已开启」。尚未提供外部预约地址，当前点击只提示开放时间；接入正式链接后替换为跳转行为。
...
- **主要内容（2026-07-14）**：顶部固定使用与其他统计卡一致的 `font-title text-xl` 标题「你的巡演城市地图」，不再为标题或文案做进场显示动画；标题下始终显示叙事——有场次时为「你去了 X 个城市…」，有可用定位和里程时补充出发地与公里数；未选场次时明确引导回上一步点亮坐标。**全部巡演城市都有标记与点标签**：未去过的是小号灰点/灰标签，去过的是大号蓝点/浅蓝标签；标签用每个 cobe marker 的 CSS anchor 与可见度变量，只在转向正面半球时显示。挂载时优先对准用户去过的第一座城市（一场未选时回落到第一座巡演城市）。
- **布局与交互**：移除底部城市胶片条、经纬度/序号与场次详情面板；地球在标题以下的剩余空间居中，保持自动旋转，不再提供点选聚焦。
- **视觉方向**：深色背景、发光地球与环形巡演文字。蓝色 `#38bdf8` 是本卡唯一强调色（呼应「蓝色的海」），用于已去过城市、出发点与星轨；轨迹弧线宽度为 `0.8`，避免多城市时显得厚重。
- **奔波距离星轨**：有可用地点时相机先对准用户出发点（`phi = 3π/2 − lng·π/180`，theta 维持 0.2），出发点为蓝色 marker + DOM 双圈脉冲环（用 cobe 的 `--cobe-user-origin` CSS anchor 锚定）；蓝色弧线（cobe v2 原生 `arcs`）从出发点连接到每座去过的城市，挂载即完整渲染，多城市自然交织成以用户为中心的引力场。出发城市名去掉行政区划后缀（浙江省→浙江），城市为「不透露」「其他国家或地区」或缺失时文案退化为「从家出发」。**启用条件**：`travelOrigin` 非空且 `mileage > 0` 且至少有一座 `isVisited` 城市（弧线目标只取 isVisited 标记，绝不连未去过的城市）；不满足时仍保留标题与「去了 X 个城市」文案。
...
- **数据状态**：地球标记、点标签与奔波距离星轨均已接入真实数据；无定位时仍保留标题和城市统计文案。
