...
- **护照首页（卡组 index 0）**：mono eyebrow `MAYDAY #5525 · TOUR PASSPORT` → Doto 大字分数「已盖章数 / 总城市数」（已盖章数白色 + 蓝辉光）→ 5 列迷你印章网格（到场 = 子主题色实线 + 低透明填充 + 辉光 + 错峰 pop-in，未到场 = 灰虚线空心；点按经 RAF 平滑翻页跳转对应城市页）→ 底部两行 MRZ 机读码（内容为真实数据：巡演名 / 盖章数 / 总城市数 / 里程，非装饰乱码）。
