...
- **主要内容**：标题「曲库内外，这是你的 5525 岁月留声机。」；引导语用三个数字连成一句话，落点「你亲耳听过的 N 张，已经替你从架上抽了出来」；引导语下方是「共振频率 VU 表盘」（`summary-vu-*`，2026-07-12 新增）——与唱片架共用木纹外框的功放面板，琥珀背光 SVG 表盘（真空管暖光意象），指针从左端甩出、过冲回摆后定格在听歌进度角度（听过张数 / 全部唱片数映射 `-50deg..50deg`，80% 起红区），百分比读数在指针收摆期间淡入，面板下文案「你与五月天的音乐共振频率」，reduced-motion 下指针静态停在进度角度；一行图例（四种唱片状态的迷你脊线色样）；上层多行唱片架 =「五月天曲库」（本巡演唱过的曲目按曲库原始顺序在前，**全巡演未唱过的灰暗唱片沉在架尾**）；下层单行金色唱片架 =「曲库外的意外惊喜」（横向滚动，复用嘉宾卡 `summary-space-scroller` 的隐藏滚动条 + 两侧 mask，仅在存在曲库外曲目时渲染）；收尾句 + 口径注脚。
