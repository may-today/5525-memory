...
- **交互**：横向 snap 滚动一屏一城（原生触摸滚动与外层纵向切卡天然共存，`|deltaY| > |deltaX|` 判定过滤横滑，无需手势豁免）；底部 dots 导航（首页为「书页」圆角短矩形，城市点按子主题首色着色、未到场灰色），dots 行 `pb-20` 给容器悬浮「滑动探索」引导让位。**已知坑：Chrome 在 `scroll-snap-type: x mandatory` 容器上会把原生 smooth 滚动（scrollIntoView / scrollTo）中途取消并停在非 snap 位置**，`scrollToIndex` 改为 RAF 自绘 ease-out 缓动 + 动画期间临时关 snap，reduced-motion 下瞬时跳转，卸载时 cancelAnimationFrame。
